...
__sphinx_build__ = False  # Define __sphinx_build__ before using it
...
lr = 1e-4  # reduced from 3e-4 for more stability
max_grad_norm = 1.0
...
#
sub_batch_size = 256  # increased proportionally with larger batch  # cardinality of the sub-samples gathered from the current data in the inner loop
...
gamma = 0.99
lmbda = 0.95
entropy_eps = 5e-3   # increased entropy bonus for enhanced exploration
...
video_recorder = VideoRecorder(logger, tag="video_v7")
...
actor_net = nn.Sequential(
    ...
    nn.LayerNorm(512, device=device),
    nn.ReLU(),
    nn.LazyLinear(512, device=device), # num_cells
    nn.LayerNorm(512, device=device),
    nn.ReLU(),
...
eval_str = ""
best_model = None
writer = SummaryWriter(comment="-ppo_lunarlander_512_wind_v7")
    ...
    if i % 10 == 0:
        ...
        with set_exploration_type(ExplorationType.DETERMINISTIC), torch.no_grad():
            ...
            if len(logs["eval reward (sum)"]) == 0 or eval_rollout["next", "reward"].sum().item() > max(logs["eval reward (sum)"]):
                ...
                model_save_path = os.path.join("./training_loop", f"ppo_lunar_policy_model_best_v7_{best_value}.pt")

...
video_recorder = VideoRecorder(logger, tag="video_v2")
...
eval_str = ""
best_model = None
writer = SummaryWriter(comment="-ppo_lunarlander_512_wind_v2")
    ...
    if i % 10 == 0:
        ...
        with set_exploration_type(ExplorationType.DETERMINISTIC), torch.no_grad():
            ...
            if len(logs["eval reward (sum)"]) == 0 or eval_rollout["next", "reward"].sum().item() > max(logs["eval reward (sum)"]):
                ...
                model_save_path = os.path.join("./training_loop", f"ppo_lunar_policy_model_best_v2_{best_value}.pt")

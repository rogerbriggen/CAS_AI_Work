...
# the better.
#
frames_per_batch = 2000  # increased from 1000
...
gamma = 0.99
lmbda = 0.95
entropy_eps = 1e-3   # increased entropy bonus for enhanced exploration
...
video_recorder = VideoRecorder(logger, tag="video_v3")
...
eval_str = ""
best_model = None
writer = SummaryWriter(comment="-ppo_lunarlander_512_wind_v4")
    ...
    if i % 10 == 0:
        ...
        with set_exploration_type(ExplorationType.DETERMINISTIC), torch.no_grad():
            ...
            if len(logs["eval reward (sum)"]) == 0 or eval_rollout["next", "reward"].sum().item() > max(logs["eval reward (sum)"]):
                ...
                model_save_path = os.path.join("./training_loop", f"ppo_lunar_policy_model_best_v4_{best_value}.pt")
